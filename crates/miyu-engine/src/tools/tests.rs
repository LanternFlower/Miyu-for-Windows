...
/// Qwen 方言下的 `parameters` 只留三段：`type` / `properties` / `required`。
fn assert_qwen_parameters_schema(schema: &serde_json::Value) {
...
/// Bailian / Qwen 方言的 `parameters` 契约：**所有**内置工具都得过。工具面里
/// 多一件带顶层 `anyOf` / `type: null` 的工具，这条就红。
///
/// 整形函数住在 miyu-core（生产路径上组请求时调用），工具面住在这一层，所以
/// 这条测试落在 engine：core 反向依赖 engine 会成环（见 core 侧注释）。
#[test]
fn all_builtin_tools_fit_the_qwen_parameters_contract() {
    let temp = tempfile::tempdir().unwrap();
    let paths = test_paths(temp.path());
    let config = miyu_base::config::AppConfig::default();
    let definitions = builtin_registry(&config, &paths).definitions();
    assert!(definitions.iter().any(|tool| tool.function.name == "alarm"));

    let provider = miyu_core::llm::qwen_tool_provider();
    for tool in miyu_core::llm::lower_chat_tools_for_qwen(&provider, definitions) {
