...
pub(in crate::web) fn start_ipc_server(
    ...
    let listener = crate::sys::ipc_bind(&socket_path)
        .with_context(|| format!("binding Miyu IPC socket at {}", socket_path.display()))?;
    crate::sys::set_mode(&socket_path, 0o600)?;
...
pub(in crate::web) async fn handle_ipc_connection(
    state: DaemonState,
    mut stream: crate::sys::IpcStream,
) -> Result<()> {
...
async fn dispatch_ipc_connection(
    ...
    mut stream: &mut crate::sys::IpcStream,
) -> Result<()> {
...
async fn client_hung_up(stream: &crate::sys::IpcStream) {
...
pub(in crate::web) async fn handle_ipc_turn(
    state: &DaemonState,
    stream: &mut crate::sys::IpcStream,
    content: String,
    mode: String,
