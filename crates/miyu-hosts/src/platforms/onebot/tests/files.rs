...
fn parses_platform_file_sources_in_priority_order() {
    #[cfg(unix)]
    let local_path = "/home/napcat/cache/clip.mp4";
    #[cfg(windows)]
    let local_path = r"C:\napcat\cache\clip.mp4";
    let data = json!({
        "file": local_path,
    ...
    assert_eq!(
        ...
        vec![
            PlatformFileSource::LocalPath(local_path.into()),
    ...
    #[cfg(unix)]
    let (file_url, file_path) = ("file:///tmp/x.bin", "/tmp/x.bin");
    #[cfg(windows)]
    let (file_url, file_path) = ("file://C:/napcat/cache/x.bin", r"C:\napcat\cache\x.bin");
    let data = json!({ "url": file_url, "file": "x.bin", "base64": "" });
    assert_eq!(
        parse_platform_file_sources(&data),
        vec![PlatformFileSource::LocalPath(file_path.into())]
    );
