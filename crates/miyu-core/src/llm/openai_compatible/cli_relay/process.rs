...
pub(in crate::llm::openai_compatible) fn kill_process_group(pid: u32) {
    miyu_base::sys::terminate_process_tree(pid, true);
}

/// 宽限期的 TERM 走 `force = false`：Unix 发 SIGTERM，Windows 侧垫片固定
/// taskkill /T（那里没有能优雅送达控制台进程的信号）。
fn signal_process_group(pid: u32, force: bool) {
    miyu_base::sys::terminate_process_tree(pid, force);
}
...
impl RelayProcess {
    ...
    async fn launch(
        ...
        let mut command = miyu_base::process::hidden_tokio_command(binary);
        command
            .args(args)
            .current_dir(workdir)
            ...
            .kill_on_drop(true);
        // 自成进程组只在 Unix 有意义（配合 killpg）；Windows 靠 taskkill /T
        // 沿子进程树走。
        #[cfg(unix)]
    ...
    pub(in crate::llm::openai_compatible) async fn terminate(&mut self) {
        signal_process_group(self.pid, false);
