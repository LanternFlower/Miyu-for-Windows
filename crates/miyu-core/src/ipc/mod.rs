...
// 平台 IPC 传输层：Unix 走 Unix domain socket，Windows 走 loopback TCP。
// 子模块（launch / lifecycle / protocol）靠 `use crate::ipc::*;` 取这里的 `sys` 与 `IpcStream`。
use miyu_base::sys::{self, IpcStream};
...
use std::path::Path;
use std::{fs::File, fs::OpenOptions, path::PathBuf, process::Stdio, time::Duration};
...
mod tests {
    ...
    ///
    /// 只在 Unix 跑：Windows 建目录符号链接要管理员/开发者模式。
    #[cfg(unix)]
    #[test]
