...
fn try_command(cmd: &str, args: &[&str], mime: &str) -> Result<Option<ClipboardImage>> {
    let output = crate::process::hidden_std_command(cmd)
        .args(args)
        .stdin(Stdio::null())
        .stdout(Stdio::piped())
        .stderr(Stdio::null())
...
fn try_write_text_command(cmd: &str, args: &[&str], text: &str) -> Result<bool> {
    let mut child = match crate::process::hidden_std_command(cmd)
        .args(args)
        .stdin(Stdio::piped())
        .stdout(Stdio::null())
        .stderr(Stdio::null())
...
pub fn parse_clipboard_path(text: &str) -> Option<ClipboardPath> {
    ...
    let path_str = if raw.starts_with('/') || Path::new(&raw).is_absolute() {
        raw.to_string()
