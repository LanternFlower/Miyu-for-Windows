...
pub(in crate::cli) fn trace_tail_redraw(
    ...
    let path = crate::platform_dirs::PlatformDirs::new()
        .map(|dirs| dirs.home_dir().join(".miyu/cache/logs/tail-trace.log"))
        .unwrap_or_else(|| std::path::PathBuf::from("tail-trace.log"));
