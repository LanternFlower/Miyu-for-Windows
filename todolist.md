...
- 子代理浮层与后台任务面板**按视口渲染**。现在是每帧重排**全部**步骤（前台 100ms、后台 150ms），所以步数上限等于每帧工作量：实测线性约 4.7ms/轮，256KB 的流水账 = 1135 轮 = debug 首帧 5.4s。这件做完，两边的步数上限才敢一起调高（09-17 用户要「回放多一些」，当时判定先不动上限）。测量脚本 `testkit/tui/bg_latency.py` 旁边那个探针思路：造一份定长日志，量首帧。
