...
fn main() {
    // The MSVC target gives the main thread a 1 MiB stack by default. Miyu's
    // async CLI dispatcher can exceed that at startup, so match the effective
    // stack headroom available on the Unix builds.
